...
# chunks, each of shape [N_MELS, CHUNK_FRAMES] = [128, 3000].
#
# Pipeline:
...
#   chunks = out.chunks  # List[Tensor[128, 3000]]
...
class AudioFrontendOutput:
    ...
    chunks : List[Tensor]
        ...
        Each tensor has shape [N_MELS, CHUNK_FRAMES] = [128, 3000].
    ...
    def stack(self) -> torch.Tensor:
        """
        Stack all chunks into a single tensor of shape [N, 128, 3000].
        Useful when you want to batch-encode all chunks through the encoders.
        """
        return torch.stack(self.chunks, dim=0)  # [N, 128, 3000]
...
class AudioFrontend:
    ...
    chunk_frames : int
        Time-frames per chunk. Default 3000 (= 30s × 100 fps).
    max_chunks : int
    ...
    def process(
        ...
        -------
        AudioFrontendOutput
            Dataclass with .chunks (List[Tensor[128, 3000]]) and metadata.
        """
